# -*- coding: utf-8 -*-
"""
Created on Mon Sep 12 17:10:19 2022
...
# for temp in np.linspace(22,50,150):
#     enviornment_setpoint_time(temp, 50, 20)

for temp in np.linspace(22,45,150):
        enviornment_setpoint_time(temp, 1, 15)
...
file_name = f'GO Sample 2 Line 3 plate temp only_{date}'

read_me = """


This is a capacitance vs. temperature test on LIG/TiO2/LIG capacitors.
It is run on the third line (or cross-section) of sample 2. The temperature 
range is 22C (~room temp) to 50C using the plate heater only. The relative 
humidity and temperature settings on the humidity chamber are turned off and
the chamber door is left open. Electrical parameters are set at 1kHz and 500mV.
Here is the driver code for the run:

for temp in np.linspace(22,50,200):
    enviornment_setpoint_time(temp, 1, 20)
...
def wind_down():    
    ...
    set_temperature(10)
#%%
print('Finished running, now on drying cycle. You may take sample out.')
ezt.set_humidity_SP(1)
time.sleep(60*20)
set_temperature(22)
#time.sleep(60*20)
